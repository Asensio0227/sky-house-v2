...
const createListingSchema = Yup.object().shape({
  ...
  depositAmount: Yup.number().when('listingType', {
    is: 'rent',
    then: (schema) => schema.min(0, 'Deposit must be 0 or greater'),
    otherwise: (schema) => schema.notRequired(),
  }),

  availableFrom: Yup.date().when('listingType', {
    is: 'rent',
    then: (schema) => schema.nullable(),
    otherwise: (schema) => schema.notRequired(),
  }),

  isFurnished: Yup.string().when('listingType', {
    is: 'rent',
    then: (schema) => schema.required('Please specify if furnished'),
    otherwise: (schema) => schema.notRequired(),
  }),

  minimumStay: Yup.number().when('listingType', {
    is: 'rent',
    then: (schema) => schema.min(1, 'Minimum stay must be at least 1 day'),
    otherwise: (schema) => schema.notRequired(),
  }),
  ...
  // Location (latitude/longitude will be added from user's location)
});
...
const CreateListing = () => {
  ...
  // Function to get fresh initial values
  const getInitialValues = () => ({
    ...
    bedrooms: '',
    bathrooms: '',
    contact_phone: currentUser?.contact_details?.phone_number || '',
    contact_email: currentUser?.email || '',
    contact_address: currentUser?.physical_address?.street || '',
    listingSource: 'owner',
  });
  ...
  const handleSubmit = useCallback(
    async (values: any, { resetForm }: any) => {
      try {
        ...
        if (!currentLocation?.latitude || !currentLocation?.longitude) {
          Alert.alert(
            'Location Required',
            'Please enable location services to create a listing'
          );
          return;
        }
        ...
        // Prepare data for submission (matching your backend model exactly)
        const listingData: any = {
          photo: values.photo, // Array of URIs
          ...
          location: {
            type: 'Point',
            coordinates: [currentLocation.longitude, currentLocation.latitude], // [lng, lat] for GeoJSON
          },
        };

        // Add listing-type specific fields
        if (values.listingType === 'sale') {
          listingData.price = Number(values.price);
        } else {
          listingData.rentPrice = Number(values.rentPrice);
          listingData.rentFrequency = values.rentFrequency;

          if (values.depositAmount) {
            listingData.depositAmount = Number(values.depositAmount);
          }
          if (values.availableFrom) {
            listingData.availableFrom = values.availableFrom;
          }
          if (values.isFurnished !== '') {
            listingData.isFurnished = values.isFurnished === 'true';
          }
          if (values.minimumStay) {
            listingData.minimumStay = Number(values.minimumStay);
          }
        }
        ...
        Alert.alert('Success!', 'Your listing has been created successfully', [
          {
            text: 'Create Another',
            onPress: () => {
              // Reset the form with fresh initial values
    ...
    [currentLocation, currentUser, dispatch, navigation]
  );
          ...
          >
            ...
            >
              <Card.Content>
                ...
                >
                  Upload 1-6 images of your property
                </Text>
                <FormImageField name='photo' maxImages={6} />
              </Card.Content>
            </Card>
